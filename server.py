...
def receive():
    ...
    while True:
        ...
        logging.info(f'[+] {nickname} connected to the server')
        print(f'[+] Nickname of the client is {nickname}')
        logging.info(f'[+] Broadcast message to all the clients that {nickname} joined the chat')
        broadcast(f'[+] {nickname} joined the chat...'.encode('ascii'))
        logging.info("[+] Connected to the server...".encode('ascii'))
        client.send("[+] Connected to the server...".encode('ascii'))
        client.send("\n\nTo Quit chat Enter 'Bye' ".encode('ascii'))
        client.send("\nEnter your message below: ".encode('ascii'))
